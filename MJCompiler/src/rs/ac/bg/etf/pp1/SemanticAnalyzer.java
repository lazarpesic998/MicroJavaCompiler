...
public class SemanticAnalyzer extends VisitorAdaptor {
	...
	public void visit(Program program) {
		...
		if (searchMain == TabS.noObj) {
			...
			if (searchMain.getKind() != Obj.Meth || searchMain.getType() != TabS.noType ||
				...
				report_error("SEMANTICKA GRESKA: MAIN metoda mora biti deklarisana kao void metoda bez argumenata",
						program);
			}
		}

		varCnt = TabS.currentScope().getnVars();
	...
	public void visit(Var varName) {
		...
		Obj searchedVar = TabS.findInScope(varName.getVarName());
	...
	public void visit(ArrayVar varName) {
		...
		if (searchedVar == TabS.noObj) {
			...
			// log info
			// varCnt++;
			report_info("Deklarisana NIZOVNA promenljiva " + varName.getVarName() + " tipa " + currentType.getKind(),
					varName);
		} else {
			report_error("Desila se SEMANTICKA GRESKA: DVOSTRUKA definicija simbola", varName);
		}
	...
	public void visit(Type typeName) {
		...
		if (typeNode == TabS.noObj) {
			report_error("SEMANTICKA GRESKA: Nije pronadjen tip " + typeName.getTypeName() + " u tabeli simbola! ",
					null);
	...
	public void visit(MethodReturnType methodReturnType) {
		if (TabS.find(methodReturnType.getMethodName()) == TabS.noObj) {
			...
			TabS.openScope();
			report_info("Deklaracija METODE : " + methodReturnType.getMethodName() + "() sa povratinim tipom: "
					+ currentType.getKind(), methodReturnType);
		} else {
			report_error("SEMANTICKA GRESKA: methoda " + methodReturnType.getMethodName() + "() je vec deklarisana",
					methodReturnType);
		}
	...
	public void visit(MethodReturnVoid methodReturnVoid) {
		...
		if (TabS.find(methodReturnVoid.getMethodName()) == TabS.noObj) {
			...
			TabS.openScope();
			report_info("Deklaracija METODE : " + methodReturnVoid.getMethodName() + "() sa povratinim tipom: "
					+ currentType.getKind(), methodReturnVoid);
		} else {
			report_error("SEMANTICKA GRESKA: methoda " + methodReturnVoid.getMethodName() + "() je vec deklarisana",
					methodReturnVoid);
		}
	}
	...
	public void visit(FormVar formVar) {
		Obj obj = TabS.findInScope(formVar.getFormVarName());
		if (obj == TabS.noObj) {
			obj = TabS.insert(Obj.Var, formVar.getFormVarName(), currentType);
			formParsCnt++;
	...
	public void visit(FormVarArray formVar) {
		// Obj searchedVar =
		// TabS.currentScope.getLocals().searchKey(formVar.getFormVarName());
		// if(searchedVar == null){
		Obj obj = TabS.findInScope(formVar.getFormVarName());
		if (obj == TabS.noObj) {
			obj = TabS.insert(Obj.Var, formVar.getFormVarName(), TabS.findArrayType(currentType));
			formParsCnt++;
			obj.setFpPos(formParsCnt);
			report_info("Deklarisan FormVar " + formVar.getFormVarName() + " tipa " + currentType.getKind(), formVar);
		} else {
	...
	public void visit(ConstNumFactor constFactor) {
		constFactor.struct = TabS.intType;
		currentArrayLenght = constFactor.getNumFactor();
	}

	public void visit(ConstCharFactor constFactor) {
		constFactor.struct = TabS.charType;
	}

	public void visit(ConstBoolFactor constFactor) {
		constFactor.struct = TabS.boolType;
	}
	...
	public void visit(ReturnStatement returnStatement) {
		...
		if (currentMethod == null) {
			report_error(
					"Desila se SEMANTICKA GRESKA: RETURN ne sme postojati izvan tela (statickih) metoda, odnosno globalnih funkcija.",
					returnStatement);
		}
		// Tip neterminala Expr mora biti ekvivalentan povratnom tipu tekuÄ‡e metode/
		// globalne funkcije.
		...
		if (!currMethType.compatibleWith(returnStatement.getExprOptional().struct)) {
			report_error(
					"Desila se SEMANTICKA GRESKA: NEKOMPATIBILNI tipovi Tip neterminala Expr mora biti ekvivalentan povratnom tipu tekuÄ‡e metode/ globalne funkcije.",
					returnStatement);
		}
	}
	...
	public void visit(FunctionCall functionCall) {
		...
		if (func.getLevel() != actParsList.size()) {
			report_error(
					"Desila se SEMANTICKA GRESKA: Broj formalnih i stvarnih argumenata metode ili konstruktora mora biti isti",
					functionCall);
		}

		// Tip svakog stvarnog argumenta mora biti kompatibilan pri dodeli sa tipom
		// svakog formalnog argumenta na odgovarajućoj poziciji.
		for (int i = 0; i < func.getLevel(); i++) {
			if (!formParsList.get(i).getType().equals(actParsList.get(i))) {
				report_error(
						"Desila se SEMANTICKA GRESKA:  Tip svakog stvarnog argumenta mora biti kompatibilan pri dodeli sa tipom svakog formalnog argumenta na odgovarajućoj poziciji",
						functionCall);
			}
		}
	...
	public void visit(DesignatorArray designatorIdent) {
		...
		DesignatorArrayStart designatorArrayStart = designatorIdent.getDesignatorArrayStart();
		designatorIdent.obj = designatorArrayStart.getDesignator().obj;
		...
		designatorIdent.obj = new Obj(Obj.Elem, designatorArrayStart.getDesignator().obj.getName(),
		...
		if (!designatorIdent.getExpr().struct.equals(TabS.intType)) {
			report_error("Desila se SEMANTICKA GRESKA: DesignatorArray: Tip neterminala Expr mora biti int",
					designatorIdent);
		}
	}
	...
	public void visit(NumConst constant) {
		...
		if (searchedConst == TabS.noObj) {
			...
			if (currentType != TabS.intType) {
				report_error("Desila se SEMANTICKA GRESKA: Tip terminala numConst mora biti ekvivalentan tipu Type.",
						constant);
			}
			...
			report_info("Deklarisana BROJEVNA KONSTANTA " + constant.getConstName() + " tipa " + currentType.getKind()
					+ " vrednosti: " + constant.getConstVal(), constant);
		} else {
	...
	public void visit(BoolConst constant) {
		...
		if (searchedConst == TabS.noObj) {
			...
			if (currentType != TabS.boolType) {
				report_error("Desila se SEMANTICKA GRESKA: Tip terminala boolConst mora biti ekvivalentan tipu Type.",
						constant);
			}

			// save
			int boolVal = 0;
			if (constant.getConstVal())
				boolVal = 1;
			Obj currentConst = TabS.insert(Obj.Con, constant.getConstName(), currentType);
			currentConst.setAdr(boolVal);

			// log info
			report_info("Deklarisana BOOL KONSTANTA " + constant.getConstName() + " tipa " + currentType.getKind()
					+ " vrednosti: " + constant.getConstVal() + ": " + boolVal, constant);
		} else {
			report_error("Desila se SEMANTICKA GRESKA: DVOSTRUKA definicija simbola", constant);
		}
	}
	...
	public void visit(CharConst constant) {
		...
		if (searchedConst == TabS.noObj) {
			// kombatiblinost tipova
			if (currentType != TabS.intType) {
				report_error("Desila se SEMANTICKA GRESKA: Tip terminala charConst mora biti ekvivalentan tipu Type.",
						constant);
			}
			...
			Obj currentConst = TabS.insert(Obj.Con, constant.getConstName(), currentType);
			currentConst.setAdr(constant.getConstVal());
			report_info("Deklarisana CHAR KONSTANTA " + constant.getConstName() + " tipa " + currentType.getKind()
					+ " vrednosti: " + constant.getConstVal(), constant);
		} else {
			report_error("Desila se SEMANTICKA GRESKA: DVOSTRUKA definicija simbola", constant);
		}
	...
	public void visit(DesignatorStatementAssign designatorStatementAssign) {
		...
		if (!(obj.getKind() == Obj.Var || obj.getKind() == Obj.Elem)) {
			report_error(
					"Desila se SEMANTICKA GRESKA: Designator mora oznacavati promenljivu, element niza ili polje unutar objekta.",
					designatorStatementAssign);
		}

		// Tip neterminala Expr mora biti kompatibilan pri dodeli sa tipom neterminala
		// Designator.
		if (designatorStatementAssign.getDesignator().obj
				.getType() != designatorStatementAssign.getAssignopExpr().struct) {
			report_error(
					"Desila se SEMANTICKA GRESKA: Tip neterminala Expr mora biti kompatibilan pri dodeli sa tipom neterminala Designator.",
					designatorStatementAssign);
		}
		if(obj.getKind() == Obj.Var && obj.getType().getKind() == Struct.Array){
			obj.setFpPos(currentArrayLenght);
		}
	...
	public void visit(DesignatorDec designator) {
		// Designator mora oznaÄ�avati promenljivu, element niza ili polje unutar
		// objekta
		// note: polje unutar objekta jos nije podrzano, klase nisu podrzane
		Obj obj = designator.getDesignator().obj;
		if (!(obj.getKind() == Obj.Var || obj.getKind() == Obj.Elem)) {
	...
	public void visit(DesignatorFunctionCall designator) {
		// Designator mora oznaÄ�avati nestatiÄ�ku metodu unutraÅ¡nje klase ili globalnu
		// funkciju glavnog programa.
		if (designator.getFunctionCall().getDesignator().obj.getKind() != Obj.Meth) {
			report_error(
					"Desila se SEMANTICKA GRESKA: Designator mora oznaÄ�avati nestatiÄ�ku metodu unutraÅ¡nje klase ili globalnu funkciju glavnog programa.",
					designator);
		}
	}

	// Svi Designator neterminali sa leve strane znaka za dodelu vrednsti moraju
	// oznaÄ�avati promenljivu, element niza ili polje unutar objekta.
	...
	public void visit(ParameterListComma parameterListComma) {
		...
		if (obj != null) {
			...
			report_info("Nadjen magic designator " + obj.getName() + " tipa " + currentType.getKind(),
					parameterListComma);
		} else {
	...
	public void visit(DesignatorMagic magic) {
		...
		if (magic.getDesignator().obj.getType().getKind() != Struct.Array) {
			report_error(
					"Desila se SEMANTICKA GRESKA: Designator sa desne strane znaka za dodelu vrednosti mora predstavljati niz..",
					magic);
		}
		/*
		 * Tip elementa niza predstavljen neterminalom Designator sa desne strane znaka
		 * za dodelu
		 * vrednosti mora biti kompatibilan pri dodeli sa tipom svih neterminala
		 * Designator sa leve strane
		...
		for (int i = 0; i < designatorMagicList.size(); i++) {
			...
			if (currElem != null && !currElem.getType().equals(tipElemenataNiza)) {
				report_error(
						"Desila se SEMANTICKA GRESKA: Tip elementa niza predstavljen neterminalom Designator sa desne strane znaka za dodelu vrednosti mora biti kompatibilan pri dodeli sa tipom svih neterminala Designator sa leve strane znaka za dodelu vrednosti.",
						magic);
			}
		}
		/*
		 * U sluÄ�aju da je sa leve strane znaka za dodelu vrednosti navedeno viÅ¡e
		 * neterminala Designator od
	...
	public void visit(Break breakFound) {
		...
		if (loopCnt <= 0) {
			report_error(
					"Desila se SEMANTICKA GRESKA: skaz break se moÅ¾e koristiti samo unutar while ili foreach petlje.",
					breakFound);
		}
	}
	...
	public void visit(Continue continueFound) {
		...
		if (loopCnt <= 0) {
			report_error(
					"Desila se SEMANTICKA GRESKA: skaz Continue se moÅ¾e koristiti samo unutar while ili foreach petlje.",
					continueFound);
		}
	}
	...
	public void visit(WhileStatement whileStatement) {
		if (loopCnt <= 0)
			report_error(
					"Desila se SEMANTICKA GRESKA: loopCnt kad se zatvara while ili foreach ne sme biti manji od 0 ",
					whileStatement);
		loopCnt--;
	}
	...
	public void visit(CondFactTwoExpr condition) {
		...
		if (!condition.getExpr().struct.equals(condition.getExpr1().struct)) {
			report_error("Desila se SEMANTICKA GRESKA: CondFactTwoExpr Tipovi oba izraza moraju biti kompatibilni",
					condition);
		}

		// Uz promenljive tipa klase ili niza, od relacionih operatora, mogu se
		// koristiti samo != i ==.
		if (condition.getExpr().struct.getKind() == Struct.Array &&
			...
			if (condition.getRelop().getClass() != Eq.class &&
				...
				report_error(
						"Desila se SEMANTICKA GRESKA: Uz promenljive tipa niza, od relacionih operatora, mogu se koristiti samo != i ==",
						condition);
			}
		}
	}
